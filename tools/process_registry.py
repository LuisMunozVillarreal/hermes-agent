...
def _systemd_run_user_scope_available():
    return _systemd_scope._systemd_run_user_scope_available()


def _is_supervised_gateway_process():
    return _systemd_scope._is_supervised_gateway_process()


def _build_systemd_scope_argv(shell_argv, unit_suffix):
    return _systemd_scope._build_systemd_scope_argv(shell_argv, unit_suffix=unit_suffix)


def _prepare_systemd_scope_argv(shell_argv, unit_suffix):
    return _systemd_scope._prepare_systemd_scope_argv(shell_argv, unit_suffix=unit_suffix)


def _stop_systemd_unit(unit_name):
    return _systemd_scope._stop_systemd_unit(unit_name)


def systemd_user_bus_env(base_env=None):
    return _systemd_scope.systemd_user_bus_env(base_env)
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _scope_argv(self, session: ProcessSession, safe_command: str, unit_suffix: str, label: str) -> List[str]:
        ...
        if in_supervised_gateway and _systemd_run_user_scope_available():
            argv, session.systemd_unit = _prepare_systemd_scope_argv(argv, unit_suffix=unit_suffix)
            return argv
